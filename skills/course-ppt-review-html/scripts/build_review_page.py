...
REQUIRED_LOGIC_FIELDS = ["label", "title", "title_zh", "text", "text_zh", "source"]
REQUIRED_TERM_FIELDS = ["term", "zh", "explanation_en", "explanation_zh", "source"]
REQUIRED_ANSWER_FIELDS = ["title", "answer_en", "answer_zh", "source"]
REQUIRED_CONFUSION_FIELDS = ["title", "wrong", "wrong_zh", "right", "right_zh", "source"]
REQUIRED_REVIEW_FIELDS = ["title", "title_zh", "items", "items_zh"]
...
def validate_spec(spec: dict[str, Any]) -> list[str]:
    ...
    for index, item in enumerate(as_list(spec.get("logic_map")), 1):
        if not isinstance(item, dict):
            errors.append(f"logic_map[{index}] must be an object")
            continue
        missing = missing_fields(item, REQUIRED_LOGIC_FIELDS)
        if missing:
            errors.append(f"logic_map[{index}] missing fields: {', '.join(missing)}")
    ...
    for index, item in enumerate(as_list(spec.get("confusions")), 1):
        if not isinstance(item, dict):
            errors.append(f"confusions[{index}] must be an object")
            continue
        missing = missing_fields(item, REQUIRED_CONFUSION_FIELDS)
        if missing:
            errors.append(f"confusions[{index}] missing fields: {', '.join(missing)}")

    for index, item in enumerate(as_list(spec.get("review_order")), 1):
        if not isinstance(item, dict):
            errors.append(f"review_order[{index}] must be an object")
            continue
        missing = missing_fields(item, REQUIRED_REVIEW_FIELDS)
        if missing:
            errors.append(f"review_order[{index}] missing fields: {', '.join(missing)}")
        if not as_list(item.get("items")):
            errors.append(f"review_order[{index}] items must be a non-empty list")
        if not as_list(item.get("items_zh")):
            errors.append(f"review_order[{index}] items_zh must be a non-empty list")
...
def default_spec() -> dict[str, Any]:
    return {
        ...
        "learner_profile": "Chinese native speaker revising an English/French-taught course; Chinese explanations are the learning scaffold, not optional decoration.",
        "learner_profile_zh": "学习者是中文母语者，英语基础较弱。页面必须用中文先帮助理解，再把内容转成可背诵英文答案。",
        "source_summary": [
            {
                "label": "Primary source",
                "text": "Source PDF or PPT name, slide range.",
                "text_zh": "来源课件或 PDF 名称、页码范围。",
        ...
        "logic_map": [
            {
                ...
                "text_zh": "用中文解释这组课件在整个模块中的作用，以及它如何帮助答题。",
                "source": "Course source: slides X-Y.",
            }
        ],
        "visuals": [],
        ...
        "review_order": [
            {
                ...
                "items_zh": ["先看懂逻辑图。", "再读图像精讲。", "最后背英文短答。"],
            }
        ],
    }
        ...
        cards.append(
            "\n".join(
                [
                    ...
                    f"  <p class=\"zh-bridge\">中文：{esc(item.get('text_zh'))}</p>" if text(item.get("text_zh")) else "",
        ...
        cards.append(
            "\n".join(
                [
                    ...
                    f"  <p class=\"zh-bridge\"><strong>{esc(item.get('title_zh'))}</strong></p>",
                    f"  <p>{esc(item.get('text'))}</p>",
                    f"  <p class=\"zh-bridge\">中文理解：{esc(item.get('text_zh'))}</p>",
                    f"  <p class=\"source-note\">{esc(item.get('source'))}</p>",
                    "</article>",
                ]
            )
        ...
        cards.append(
            "\n".join(
                [
                    ...
                    f"    <p class=\"zh-bridge\">中文理解：{esc(item.get('what_zh'))}</p>",
                    "    <div class=\"explain-list\">",
                    f"      <div class=\"explain-item\"><b>How to read / 怎么看</b><span>{esc(item.get('how'))}</span><span class=\"zh-bridge\">中文：{esc(item.get('how_zh'))}</span></div>",
                    f"      <div class=\"explain-item\"><b>Must remember / 必须记</b><span>{esc(item.get('remember'))}</span><span class=\"zh-bridge\">中文：{esc(item.get('remember_zh'))}</span></div>",
                    f"      <div class=\"exam-line\">Exam sentence: {esc(item.get('exam_sentence'))}<div class=\"exam-zh\">中文：{esc(item.get('exam_sentence_zh'))}</div></div>",
                    f"      <div class=\"explain-item\"><b>Do not confuse / 别混淆</b><span>{esc(item.get('dont_confuse'))}</span><span class=\"zh-bridge\">中文：{esc(item.get('dont_confuse_zh'))}</span></div>",
                    "    </div>",
                    f"    <p class=\"source-note\">{esc(item.get('source'))}</p>",
                    "  </figcaption>",
                    "</figure>",
        ...
        cards.append(
            "\n".join(
                [
                    ...
                    f"  <p class=\"zh-bridge\"><strong>中文误区：</strong>{esc(item.get('wrong_zh'))}</p>",
                    f"  <p><strong>Exam-safe version:</strong> {esc(item.get('right'))}</p>",
                    f"  <p class=\"zh-bridge\"><strong>中文纠正：</strong>{esc(item.get('right_zh'))}</p>",
                    f"  <p class=\"source-note\">{esc(item.get('source'))}</p>",
                    "</article>",
                ]
            )
...
def render_review_order(spec: dict[str, Any]) -> str:
    ...
    for item in as_list(spec.get("review_order")):
        ...
        items_zh = "".join(f"<li>{esc(point)}</li>" for point in as_list(item.get("items_zh")))
        cards.append(
            "\n".join(
                [
                    ...
                    f"  <h3>{esc(item.get('title'))}</h3>",
                    f"  <ul>{items}</ul>",
                    f"  <h4>{esc(item.get('title_zh'))}</h4>",
                    f"  <ul class=\"zh-bridge\">{items_zh}</ul>",
                    "</article>",
                ]
            )
        )
...
<main class="wrap">
  <section class="hero">
    <div class="hero-main">
      ...
      <p class="zh-bridge">{esc(spec.get('learner_profile_zh'))}</p>
...
def collect_metrics(html_path: Path) -> HtmlMetrics:
    ...
    return HtmlMetrics(
        ...
        chinese_ratio=round(chinese_chars / visible_chars, 4) if visible_chars else 0.0,
        ...
        zh_blocks=classes.count("zh")
        + classes.count("cn")
        + classes.count("translation")
        + classes.count("zh-bridge")
        + classes.count("exam-zh"),
...
def audit_metrics(metrics: HtmlMetrics, args: argparse.Namespace) -> list[str]:
    ...
    if args.require_chinese and metrics.chinese_ratio < args.min_chinese_ratio:
        failures.append(f"chinese_ratio={metrics.chinese_ratio:.4f} < required {args.min_chinese_ratio:.4f}")
...
def add_audit_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--min-zh-blocks", type=int, default=20)
    parser.add_argument("--min-source-labels", type=int, default=12)
    parser.add_argument("--min-chinese-chars", type=int, default=1200)
    parser.add_argument("--min-chinese-ratio", type=float, default=0.12)
